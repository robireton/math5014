...
% \setallmainfonts(Digits,Latin){Alegreya}
\usepackage{Alegreya}
...
\begin{definition}
	...
	\end{enumerate}
	Exercises 1.1, 1.2, and 1.3 show that $S_X$ is a group with composition as operation; it is called the \term{symmetric group} on $X$.
	When $X=\{1, 2, \dots, n\}$, then $S_X$ is denoted $S_n$ and it is called the \term{symmetric group on n letters}.
\end{definition}

\begin{definition}
	A pair of elements $a$ and $b$ in a semigroup \term{commutes} if $a\ast b=b\ast a$. A group (or a semigroup) is \term{abelian} if every pair of its elements commutes.
\end{definition}

\begin{theorem}
	If $G$ is a group, there is a unique element $e$ with $e\ast a=a=a\ast e$ for all $a\in G$. Moreover, for each $a\in G$, there is a unique $b\in G$ with $a\ast b=e=b\ast a$.
\end{theorem}
...
\begin{definition}
	The \term{Euler 𝝋-function} is defined as follows:
